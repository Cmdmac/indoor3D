...
IndoorMap2dEx = function(mapdiv){
    ...
    this.init = function(){
        ...
        _this.containerSize[1] = parseInt(_mapDiv.clientHeight);
...
Canvas2DRenderer = function (map) {
    ...
    this.render = function (){
        ...
        //draw floor, draw every line
        var poly = _curFloor.newOutline;
        for(var i = 0; i < poly.length - 1; i+=4){
            _ctx.beginPath();
            // console.log("line:" + poly[0] + "," + (-poly[1]) + "-" + poly[i] + "," + (-poly[i+1]))
            _ctx.moveTo(poly[i], -poly[i + 1]);
            _ctx.lineTo(poly[i + 2],-poly[i + 3]);
